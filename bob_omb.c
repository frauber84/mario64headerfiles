...
void _start ( void )
{
    ...
    if (*ObjectFlag == 0)
    {
        ...
        *CurrentObject = (void*)M64_FIRST_OBJ_STRUCT;
        ...
        for (i = 0; i < 240; i++)
        {          
            if ( (*Obj)->behavior_script_entry == goomba_behav )
            {
                if ( ( (*Obj)->active != 0 )  ||  ( (*Obj)->y_pos > 1930 )  ||  ( (*Obj)->action < 0x60 )   )
                {
                    GoombaCount++;
                }
            }
...
//        PrintInt(100, 100, "%d", GoombaCount);  // debug
    ...
    else if (*ObjectFlag == 1 || *ObjectFlag == 2)
    {
        ...
        if ( (*Obj)->action == 0 && *ObjectFlag == 2) // dialog has ended
        {
            ...
            ChildObject->behav_param2 = 0x02000000;
