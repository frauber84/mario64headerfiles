...
//    PrintInt(10, 40, "GROUND Y %d", (int)Mario->_0x70);
    ...
    if (*Counter > FIRE_RATE )
    {
        ...
        if  (*pad & 1 && ( Mario->action != 0x8008a9 || Mario->action != 0x80023C ) )  // check pads and if not ground pounding
        {
           ...
           if (Mario->action & 0x30000000)   // on water
            {
                ChildObj->_0xb8 = 5;
                Mario->action = 0x300024E1;
            }
            else
            {
                Mario->action = 0x800457;   // when not on water, punch animation     
            }
