...
void _start ( void )
{
    ...
    *nopbyte = 0x00000000;
    ...
    if (*INIT == 0)
    {
        ...
        *ptr = 0x80750010;
        ptr++;
        *ptr = 0x80750010;
        DmaCopy(0x80700000, 0x1700000, 0x1740000);  
        DmaCopy(0x80740000, 0x1740000, 0x1780000); 
        
        Object *NewObject = SpawnObj( (*Obj), 0xbe, 0x13002a48);  
        NewObject->geo_layout_ptr = ProcessGeoLayout(0x80780000, 0x80742DD4); 
        NewObject->x_pos += 75;
    ...
    *CurrentObject = Yoshi;
    ...
    /* gravity */
    int (*function)(void) = (void*)0x802e4e90;
    function();
    *CurrentObject = pseudostack;
    ...
    Pad *Controller2 = (void*)0x8033AFAC;
       ...
       else if (*CameraOnObject == 1)
       {
           *ObjectDistanceSource = Yoshi; // calculate distances
           Camera *camera = Mario->camera;     
           camera->x = (Yoshi->x_pos + Mario->x_pos)/2;
           camera->y = (Yoshi->y_pos + Mario->y_pos)/2;
           camera->z = (Yoshi->z_pos + Mario->z_pos)/2;
           PrintInt(110, 20, "middle", 0);
       }
...
void ControlYoshi()
{
        ...
        int angle = (int)Yoshi->y_rotation/180 + 180;
        int dir = 0;
        ...
        {
            Yoshi->y_rotation = _8037a9a8( -(Controller2->y), Controller2->x);
            Yoshi->y_rotation2 = _8037a9a8(-(Controller2->y), Controller2->x);        
        }
     ...
     float distanceObj = DistanceFromObject(Yoshi, Mario->MarioObj);
        ...
        {
            case 1:
                Yoshi->z_pos -= (int)Controller2->y * 0.4f * cosf( angle *PI/180.0);
                Yoshi->x_pos -= (int)Controller2->y * 0.4f * sinf( angle *PI/180.0);
                break;

            case 2:
                Yoshi->z_pos += (int)Controller2->x * 0.4f * cosf( angle *PI/180.0);
                Yoshi->x_pos += (int)Controller2->x * 0.4f * sinf( angle *PI/180.0);
                break;
        ...
        if (Controller2->currentButton & BUTTON_A && *speed == 0)
        {
            Yoshi->y_speed += 5;
            
            if (Yoshi->y_speed > 25) *speed = 1;
        }
